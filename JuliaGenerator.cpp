...
JuliaGenerator::JuliaGenerator(std::size_t max_iteration) :
...
unsigned char*
JuliaGenerator::generate(std::size_t size_x, std::size_t size_y)
{
    ...
    /*
    // utiliser std::views::iota & les ranges quand tout sera dispo
...
JuliaGenerator::compute_value(float x, float y, float zoom_x, float zoom_y) noexcept
{
    //const float c_r = 0.285f;
    //const float c_i = 0.01f;
    const float c_r = -0.123f;
    const float c_i = 0.745f;
    float z_r = x / zoom_x + _x_begin;
    float z_i = y / zoom_y + _y_begin;
    unsigned int i = 0;
